...
for i in range(n_models):
    ...
    chunk = male_reg_att[start:end]


    idx = pd.MultiIndex.from_product(
        [range(n_data), range(10, 0, -1)],
...
for i in range(n_models):
    ...
    chunk = female_reg_att[start:end]
...
def plot_attention_regression(
    ...
    mx_flat  = _flatten3d(mx)
    att_flat = _flatten3d(att)
    ...
    if N_att != N_mort:
        if N_att % N_mort == 0:
            k = N_att // N_mort
            mx_flat = np.repeat(mx_flat, k, axis=0)
        elif N_mort % N_att == 0:
            k = N_mort // N_att
            att_flat = np.repeat(att_flat, k, axis=0)
        else:
    ...
    for idx, age in enumerate(ages_step):
        ...
        for s in range(n_lags):
            x_vals = mx_flat[:,  s, age]
            y_vals = att_flat[:, s, age]
...
def plot_attention_contribution(
    ...
    mx_flat  = _flatten3d(mx)
    att_flat = _flatten3d(att_contribution)
