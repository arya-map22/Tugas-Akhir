...
if RUNTIME_ENVIRONMENT == RuntimeEnvironment.colab:
    ...
    # Pindah ke direktori kerja Colab.
...
from torch.optim import NAdam, Optimizer
from torch.optim.lr_scheduler import CosineAnnealingWarmRestarts, LinearLR, SequentialLR
from torch.utils.data import ConcatDataset, DataLoader

from ta_module.actuarial import (
    compute_ex,
...
from ta_module.data import NormalizedMortalityDataset, get_train_val_test_split
from ta_module.models import (
    EnsembleLocalGLMNet,
    LocalGLMnet,
...
mortalitas_df = pd.read_csv(
    DOT_ENV.mortalitas_file, parse_dates=[YEAR_COL], date_format=MORTALITAS_CONFIG.date_format, sep=";", decimal=","
)

populasi_df = pd.read_csv(DOT_ENV.populasi_file, sep=";", decimal=",")

bi_rate_df = pd.read_csv(
    DOT_ENV.bi_rate_file, parse_dates=[BI_RATE_CONFIG.date_col], date_format=BI_RATE_CONFIG.date_format, sep=";", decimal=","
)
# %%
...
def plot_usia_vs_tahun(
    ...
    _palette = palette or ["#2E6FA3", "#D4704F"]
...
# Deteksi epoch restart.: puncak lokal di cosine annealing dengan LR > 4.98e-3
...
if CONFIG.mode in [ModeEnum.TRAIN, ModeEnum.TUNE]:
    ...
    # Metadata dipakai untuk memuat checkpoint terakhir secara otomatis.
    # Jika proses training tidak dijalankan.
...
if not male_simulations_file_path.exists():
    ...
    # Kembalikan skala data ke rentang (0, 2).
...
def sampling_mortality_trend(
    ...
    x = torch.arange(0, 3).expand(n_sims, -1).contiguous().to(device)
    ...
    return samples.expand(n_sims, n_samples, -1).contiguous()
# %%
